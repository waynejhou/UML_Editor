...
public class CompositionLineElement extends LineElement {
	...
	protected void HowToDraw(Graphics2D g, Point o) {
		...
		double d = Math.sqrt((Math.pow((getPt1().x-getPt2().x), 2) +
				Math.pow((getPt1().y-getPt2().y), 2)));
		double mx = (getPt1().x-getPt2().x)/d*20;
		double my = (getPt1().y-getPt2().y)/d*20;
		...
		int mmx= (int)(mx*2+getPt2().x);
		int mmy= (int)(my*2+getPt2().y);
		g.drawLine(getPt1().x + o.x, getPt1().y + o.y, getPt2().x + o.x, getPt2().y + o.y);
		g.fillPolygon(
				new int[] {getPt2().x+ o.x, lx+ o.x,mmx+o.x,rx+ o.x},
				new int[] {getPt2().y+ o.y, ly+ o.y,mmy+o.y,ry+ o.y},
				4);
